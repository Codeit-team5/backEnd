...
async function register(groupId,post) {
  ...
  const hashedPassword = hashPassword(post.postPassword, salt);
  const hashAlternative = {
    ...post,
    postPassword: hashedPassword,
    salt: salt
  };
  const createPost = await postRepository.create(hashAlternative,groupId,newPostId);
...
async function compare(postId,sendPassword){
  ...
  //Password 저장하기
  const restorePassword = await postRepository.findByPasswordAndSalt(postId);
  const hashedInput = hashPassword(sendPassword.password, restorePassword.salt);
  if (hashedInput === restorePassword.postPassword){
    return true;
  }
  else{
    return false;
...
async function fixPost(postId, newPost){
  const groupPass = await postRepository.findByPasswordAndSalt(postId);
  ...
  if(newPost.nickname == null ||
    ...
    //비밀 번호가 같지 않을 때
  const hashedInput = hashPassword(newPost.postPassword, groupPass.salt);
  if(hashedInput !== groupPass.postPassword){
    return "wrongFixPostPassword";
  }

  //새로운 salt 추가
  const newSalt = crypto.randomBytes(16).toString('hex');
  const newHashedPassword = hashPassword(newPost.postPassword, newSalt);
  newPost.postPassword = newHashedPassword;
  newPost.salt = newSalt;
...
async function deletePost(postId, postPassword){
  const deletedPost = await postRepository.findByPasswordAndSalt(postId);

  //확인하기
  const hashedInput = hashPassword(postPassword, deletedPost.salt);
  if(hashedInput !== deletedPost.postPassword){
